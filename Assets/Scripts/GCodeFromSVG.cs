...
public class GCodeFromSVG : MonoBehaviour
{
    ...
    public static string convertFromText(string svgText, bool replaceUnitByPixel = false)
    {
        ...
        {
            cncLogger.Log(e+ " error loading SVG-Code");
            cncLogger.Log("Error '" + e.ToString() + "' in XML string "); return "( No valid SVG data found)";
        }
    }
    ...
    {
        cncLogger.Log("convertSVG "+txt);

        svgBezierAccuracy = CNC_Settings.importBezierLineSegmentsCnt;
        svgMaxSize = CNC_Settings.importSVGMaxSize;
        svgClosePathExtend = CNC_Settings.importSVGPathClose;
        svgConvertToMM = CNC_Settings.importSVGToMM;                  // Target units and display in setup
        svgGroupObjects = CNC_Settings.importSVGGroups;           // SVG-Import group objects
        svgNodesOnly = CNC_Settings.importSVGNodesOnly;
    ...
    {   // One px unit is defined to be equal to one user unit. Thus, a length of "5px" is the same as a length of "5".
        ...
        if (svgCode.Attribute("viewBox") != null)   // viewBox unit always in px
        {
            ...
            //cncLogger.Log(string.Format("{0} {1} {2} {3}",vbOffX,vbOffY,vbWidth,vbHeight));
        }
