...
/// <summary>
/// Parabola controller  Originally by https://github.com/ditzel.
/// Modified by https://github.com/mmalherbe.
/// </summary>
...
public class ParabolaController : MonoBehaviour
{
    ...
    void OnDrawGizmos()
    {
        if (gizmo == null)
        {
            gizmo = new ParabolaFly(ParabolaRoot.GetComponent<RectTransform>());
        }

        gizmo.RefreshRectTransforms(1f);
    ...
    {
        parabolaFly = new ParabolaFly(ParabolaRoot.GetComponent<RectTransform>());

        if (Autostart)
        {
            RefreshRectTransforms(Speed);
            FollowParabola();
        }
    }
    ...
    public RectTransform[] getPoints()
    {
    ...
    /// </summary>
    public void RefreshRectTransforms(float speed)
    {
        parabolaFly.RefreshRectTransforms(speed);
    }
    ...
    public class ParabolaFly
    {
        ...
        public ParabolaFly(RectTransform ParabolaRoot)
        {

            List<Component> components = new List<Component>(ParabolaRoot.GetComponentsInChildren(typeof(RectTransform)));
            List<RectTransform> transforms = components.ConvertAll(c => (RectTransform)c);

            transforms.Remove(ParabolaRoot);
            transforms.Sort(delegate (RectTransform a, RectTransform b)
            {
